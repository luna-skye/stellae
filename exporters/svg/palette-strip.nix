...
let
  ...
  mkTokenRect = x: y: cat: name: /* html */ ''<rect id="${name}" width="${toString options.boxSize.w}" height="${toString options.boxSize.h}" x="${toString (x * options.boxSize.w)}" y="${toString (y * options.boxSize.h)}" fill="${hex element.${cat}.${name}}" />'';

  svg = /* html */ ''
    <!--
    ${helpers.mkStellaeStamp "SVG Palette Strip" element.name}
    -->

    <svg width="${toString (helpers.math.round (options.boxSize.w * 24))}" height="${toString (helpers.math.round (options.boxSize.h))}" xmlns="http://www.w3.org/2000/svg">
      <g>
...
in pkgs.stdenv.mkDerivation {
  name = "svg-palette-strip";
  src = ./.;
  buildPhase = ''
    mkdir -p "$out/${element.name}/svg"
    echo '${svg}' > "$out/${element.name}/svg/palette-strip.svg"
  '';
}
