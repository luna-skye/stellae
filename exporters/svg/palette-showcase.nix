...
{ element, options ? {
  ...
  inherit (helpers) math;
  hex = hex: "#${helpers.hslToHex hex}";
  ...
  svgWidth = math.round ((options.boxSize.w * 24) + (options.boxPadding * 2));
  svgHeight = math.round ((options.boxSize.h * 2) + (options.boxPadding * 2));
